"""
backend/pipeline_core.py — Readability-controlled summarisation pipeline.
...
CHANGE FROM v1: The OpenAI API key is no longer sourced from request.api_key.
It is loaded from settings.openai_api_key (the OPENAI_API_KEY environment
variable set on the Render server). No other logic has changed.
"""
...
async def run_pipeline(request: PipelineRequest) -> PipelineResponse:
    ...
    Steps
    -----
    1.  Initialise OpenAI client using the server-side API key from settings.
        (Previously sourced from request.api_key — now from environment.)
    ...
    request : PipelineRequest
        Validated FastAPI request payload (no api_key field).
    ...
    # ── Initialise client from server-side environment key ──
    # CHANGED: was request.api_key — now settings.openai_api_key (env var)
    client = AsyncOpenAIClient(
